...
func Run(conn db.Conn, dk docker.Client) {
	...
	for {
		...
		log.WithError(err).Debug("Failed to get Kubernetes pod watcher. This " +
			"is expected while the kube-apiserver container is first booting.")
		time.Sleep(5 * time.Second)
	}
	for {
	...
	secretClient := secretClientImpl{clientset.CoreV1().Secrets(corev1.NamespaceDefault)}
	go func() {
...
func NewKubeconfig(server string) api.Config {
	...
	apiConfig.Clusters["kelda"].CertificateAuthority = tlsIO.CACertPath(cliPath.MinionTLSDir)
	apiConfig.Clusters["kelda"].Server = server

	apiConfig.AuthInfos["tls"] = api.NewAuthInfo()
	apiConfig.AuthInfos["tls"].ClientCertificate = tlsIO.SignedCertPath(cliPath.MinionTLSDir)
	apiConfig.AuthInfos["tls"].ClientKey = tlsIO.SignedKeyPath(cliPath.MinionTLSDir)
