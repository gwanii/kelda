		...
		if err == nil {
			desiredContainers = append(desiredContainers, docker.RunOptions{
				...
				Mounts: []dkc.HostMount{
					{
						...
						Type:   "bind",
						// The Kubelet sometimes creates mounts inside
						// /var/lib/kubelet (e.g. it creates a tmpfs mount for
						// secret volumes). In order for the mount to propagate
						// to within the Kubelet container, the propagation
				...
				FilepathToContent: map[string]string{
					"/var/lib/kubelet/kubeconfig": string(kubeconfigBytes),
				},
			})
		} else {
