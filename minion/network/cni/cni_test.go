...
func TestCmdDelResult(t *testing.T) {
	...
	labelErr := anErr
	getPodLabels = func(podName string) (map[string]string, error) {
		return map[string]string{"keldaIP": "1.2.3.4"}, labelErr
	}
	...
	labelErr = nil
	err = cmdDel(&args)
	assert.EqualError(t, err, "failed to teardown OVS ports: err")

	mockErr = nil
	mk.On("LinkByName", "1.2.3.4").Once().Return(nil, anErr)
	err = cmdDel(&args)
	assert.EqualError(t, err, "failed to find outer veth: err")

	link := &netlink.GenericLink{}
	mk.On("LinkByName", "1.2.3.4").Return(link, nil)
...
func TestCmdAddResult(t *testing.T) {
	...
	mk.On("AddVeth", "1.2.3.4", "-1.2.3.4", mtu).Once().Return(anErr)
	_, err = cmdAddResult(&args)
	assert.EqualError(t, err, "failed to create veth 1.2.3.4: err")

	mk.On("AddVeth", "1.2.3.4", "-1.2.3.4", mtu).Return(nil)
	...
	_, err = cmdAddResult(&args)
	assert.EqualError(t, err, "failed to setup OVS: failed to configure OVSDB: err")
...
func TestSetupOVS(t *testing.T) {
	execRun = func(name string, args ...string) error {
		return errors.New("execRun")
	}

	ip := net.ParseIP("1.2.3.4")
	mac, _ := net.ParseMAC(ipdef.IPToMac(ip))
	err := setupOVS("outer", ip, mac)
	assert.EqualError(t, err, "failed to configure OVSDB: execRun")
