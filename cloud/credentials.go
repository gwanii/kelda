...
func syncCredentialsOnce(conn db.Conn, sshKey ssh.Signer, ca rsa.KeyPair, kubeSecret string) {
	...
	needsKubeSecret := func(m db.Machine) bool {
		...
		return !ok && m.Status != db.Stopping && m.PublicIP != "" && m.Role == db.Master
	}
	for _, m := range conn.SelectFromMachine(needsKubeSecret) {
		...
		if err := installKubeSecret(m, sshKey, kubeSecret); err == nil {
			...
			log.WithError(err).WithField("host", m.CloudID).Error("Failed to install Kubernetes secret")
		}
	}
}
